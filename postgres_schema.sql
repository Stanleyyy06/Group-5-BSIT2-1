...
def create_user(username, password_hash, role="user", approved=False):
    execute(
        ...
        (username, password_hash, role, int(approved)),
        commit=True,
    )
...
def create_allowed_ip(ip, label=None, approved_by="system"):
    ...
    execute(
        ...
        (ip, label or "approved", 1, approved_by, now, now),
        commit=True,
    )
...
def disable_allowed_ip(ip):
    ip = normalize_ip(ip)
    execute(
        "UPDATE allowed_ips SET active = 0 WHERE ip = ?", (ip,), commit=True
    )
...
def block_ip(ip, reason, blocked_by="system"):
    ...
    if existing:
        execute(
            "UPDATE blocked_ips SET active = 1, reason = ?, blocked_by = ?, blocked_at = ? WHERE ip = ?",
            (reason, blocked_by, datetime.utcnow(), ip),
            commit=True,
        )
    else:
        execute(
            "INSERT INTO blocked_ips (ip, reason, active, blocked_by, blocked_at) VALUES (?, ?, ?, ?, ?)",
            (ip, reason, 1, blocked_by, datetime.utcnow()),
            commit=True,
        )
...
def unblock_ip(ip):
    ip = normalize_ip(ip)
    execute(
        "UPDATE blocked_ips SET active = 0 WHERE ip = ?", (ip,), commit=True
    )
...
def create_login_request(username, ip, device_info=None):
    ...
    execute(
        ...
        (username, ip, device_info or "Unknown device", "pending", datetime.utcnow(), datetime.utcnow()),
        commit=True,
    )
    return execute(
...
def update_login_request(request_id, status, admin_notes=None):
    execute(
        ...
        (status, admin_notes or "", datetime.utcnow(), request_id),
        commit=True,
    )
...
def create_notification(title, message, level="info", target_role="admin"):
    execute(
        ...
        (title, message, level, target_role, 0, datetime.utcnow()),
        commit=True,
    )
...
def mark_notification_read(notification_id):
    execute(
        "UPDATE notifications SET is_read = 1 WHERE id = ?", (notification_id,), commit=True
    )
...
def record_log(user_id, username, ip, event, category="system", success=False):
    ...
    execute(
        ...
        (user_id, username, ip, event, category, int(success), datetime.utcnow()),
        commit=True,
    )
...
def count_recent_failed_attempts(ip, window_minutes=15):
    ...
    row = execute(
        ...
        (ip, "Failed authentication", datetime.utcnow() - timedelta(minutes=window_minutes)),
        fetchone=True,
    )
    return _scalar(row)
...
def fetch_dashboard_metrics():
    ...
    metrics["blocked_ips"] = _scalar(execute("SELECT COUNT(*) AS total FROM blocked_ips WHERE active = 1", fetchone=True)) or 0
    metrics["unread_notifications"] = _scalar(
        execute("SELECT COUNT(*) AS total FROM notifications WHERE target_role = ? AND is_read = 0", ("admin",), fetchone=True)
    ) or 0
    metrics["recent_security_events"] = _scalar(
        execute("SELECT COUNT(*) AS total FROM logs WHERE category = 'security' AND created_at >= ?", (datetime.utcnow() - timedelta(hours=24),), fetchone=True)
    ) or 0
    return metrics
